...
let sessionBlinkData=[];
let sessionBandsData=[];
/**
 * This class handle:
...
class Cortex {
    ...
    unSubscribe(stream,session,auth){
        let socket = this.socket
    ...
    getSessionData(){
        let sessionData = {sessionBlinkData,sessionBandsData};
        sessionBlinkData =[];
        sessionBandsData =[];
        return sessionData;
    }
            ...
            try {
                ...
                if(parsedData.pow){
                    ...
                    allArray.forEach((value)=>{
                        ...
                        meanAlpha=meanAlpha+value.alpha
                        meanBeta=meanBeta+value.beta
                        meanGamma=meanGamma+value.gamma
                    });
                    // console.log(meanAlpha)
                    meanAlpha=meanAlpha/allArray.length;
                    meanBeta=meanBeta/allArray.length;
                    ...
                    // alpha = Math.abs((Math.log(parsedData.pow[1]) - Math.log(parsedData.pow[66]))/(Math.log(parsedData.pow[1]) + Math.log(parsedData.pow[66]))); 
                    // console.log(alpha)
    ...
    getDetectionInfo(){
        ...
        // console.log(mentalCommandActiveActionRequest)
        return new Promise(function(resolve, reject){
            socket.send(JSON.stringify(detection))
            socket.on('message', (data)=>{
                let action = JSON.parse(data);
                
                if(moment().format("HH:mm:ss")===endTime){
                    let meanBlinkDuration=0;
                    if(blinkDuration.length>0){
                        blinkDuration.forEach(d=> {
                            meanBlinkDuration +=d;
                        });
                        meanBlinkDuration =meanBlinkDuration/blinkDuration.length;
                    }
                    sessionBlinkData.push({
                        count,
                    ...
                    if(Array.isArray(action.fac) && action.fac.length>0){

                        if(action.fac[0] ==='blink'){
                            if(!prev){
                                console.log('blink Start',action.fac[0])
                                blinkStart = moment();
                            }
                            prev=action.fac;
                        }else{
                            if(prev){
                                let duration = moment.duration(moment().diff(blinkStart)).milliseconds();
                                count++;
    ...
    async querySessionInfo(){
        ...
        let authToken=""
        await this.authorize().then((auth)=>{authToken = auth}).catch(err=>console.log(err))
    ...
    sub(streams){
        this.socket.on('open',async ()=>{
            ...
            let data = this.socket.on('message', (data)=>{
