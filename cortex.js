...
class Cortex {
    ...
    getSessionData(){
        let sessionData = sessionBlinkData;
        sessionBlinkData =[];
        return sessionData;
    }
    ...
    subRequest(stream, authToken, sessionId){
        ...
        socket.on('message', (data)=>{
            ...
            try {
                ...
                if(parsedData.pow){
                    console.log('here')
                    alpha = (parsedData.pow[1]+parsedData.pow[66])/2;
                    beta = (parsedData.pow[2]+parsedData.pow[3]+parsedData.pow[67]+parsedData.pow[68])/4;
                    gamma=(parsedData.pow[4]+parsedData.pow[69])/2;
                    theta=(parsedData.pow[0]+parsedData.pow[65])/2;
                    allArray.push({ alpha,beta,theta,gamma });

                    if (allArray.length===50 ) { 
                        // console.log('here')
                        let meanAlpha=0
                        let meanBeta=0
                        let meanTheta=0
                    ...
                    meanAlpha=meanAlpha/100;
                    meanBeta=meanBeta/100;
                    meanGamma=meanGamma/100;
                    meanTheta=meanTheta/100;
